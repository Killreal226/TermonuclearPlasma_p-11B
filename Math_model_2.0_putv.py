...
def time_kylon (T_electron:float, concentr_electron: float):
    '''
        Находит время кулоновского замедления 
            для дальнейшего нахождения
         концентрациии быстрых альфа-частиц
    '''
    time_kyl = ((12*math.pi**2/math.sqrt(2*math.pi))*(weight_He/math.sqrt(weight_electron))*
                ((e_0**2)*((k*(T_electron/8.61e-8))**1.5)/(concentr_electron*log_K*(charge_electron**4)*charge_He**2)) )
    return (time_kyl)
def concentr_fast (data_val:list, speed_react:float,concentr_B: float,time_kyl:float,speed_brake:float):
    '''
        Находит концентрацию быстрых альфа частиц
    '''
    concentr_fast_He = ((speed_react*data_val[0]*concentr_B*time_kyl)*
                        ((1/3)*(math.log(1+(math.sqrt((2*energy_He1*1.6e-13)/weight_He)/speed_brake)**3))+
                         (2/3)*(math.log(1+(math.sqrt((2*energy_He2*1.6e-13)/weight_He)/speed_brake)**3))))
    fraction_fast_He = concentr_fast_He/data_val[0]
    return (fraction_fast_He)
...
def speed_reaction (data_val:list,T_ion:int,concentr_B:float):
    ...
    speed_react = (speed_react_NR + speed_react_R)*(0.0009*T_ion+1.0043)                                 #Скорость реакции суммарная
...
def alpha_ion_electron (data_val:list,T_electron:float,concentr_electron:float,concentr_B:float):
    ...
    return (alpha_ion,alpha_electron,speed_brake)
...
def T_electron_detect (data_state:list,T_ion:int,T_electron:float):
    ...
    alpha_ion, alpha_alectron, speed_brake = alpha_ion_electron (data_state,T_electron,concentr_electron,concentr_B)
...
def coeff_gain_power_Pfus_Pb_func (data_state:list,T_ion:int, T_electron:float):
    ...
    time_kyl = time_kylon (T_electron, concentr_electron)
    fraction_fast_He = concentr_fast(data_state,speed_react,concentr_B,time_kyl,speed_brake)
    return (coeff_gain_power,P_fus_Pb,fraction_He, fraction_fast_He)
...
def coeff_gain_power_main (data_val:list,):
    ...
    for data_state in data_val:
        ...
        for data_temp in data_state[1]:
            coeff_gain_power,Pfus_Pb,fraction_He, fraction_fast_He = coeff_gain_power_Pfus_Pb_func (data_state[0],data_temp[0],data_temp[1])
            array_temp_Q = [data_temp[0],data_temp[1],coeff_gain_power,Pfus_Pb,fraction_He, fraction_fast_He]
...
if __name__ == '__main__':
    ...
    with open('Temp_electron_Putv.txt', 'w') as f:
        f.write(str(data_new))
        
    with open('Temp_electron_Putv.txt', 'r') as f:
        data_ = eval(f.read())

    data_new = coeff_gain_power_main (data_)                                #Модуль создания списка с коэффициентом усиления мощности

    with open('T_el__coeff_power__Pfus_pb__XHe_Putv.txt', 'w') as g:
